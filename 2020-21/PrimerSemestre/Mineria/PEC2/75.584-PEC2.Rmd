...
Una vez que tenemos todas las medidas, debemos saber que a mayor distancia a los centros de cada cluster es mejor y a menor distancia entre los puntos de un clúster a su centro también es mejor, por lo que a priori presenta una mayor calidad del modelo cuando tenemos 6 clústers.

Es por ello, que vamos a analizar los resultados obtenidos en primer lugar para un modelo con 5 clústers y luego para un modelo con 6 clústers.
...
Cabe destacar que si en un gráfico hay grupos mezclados que a priori no tendría mucho sentido, es porque falta analizar el resto de gráficos (diferentes puntos de vista), por lo que ahora vamos a sacar unas primeras conclusiones y al final de todo el análisis tendremos ya las conclusiones definitivas.

Del anterior gráfico, podemos ver como es lógico que tenemos 5 grupos ya que estamos analizando primero el modelo kmean con una k igual a 5.
...
Sin embargo dos grupos se mezclan (verde y azul claro), los primeros son gente de mediana/tercera edad con unos ingresos medios/elevados y los segundo son gente joven con unos ingresos medios/elevados.
...
- Aquellos jóvenes de una edad superior a los anteriores con unos gastos elevados. (grupo azul claro)
...
- Gente de mayoritariamente de mediana edad aunque también de tercera edad con unos gastos medios/bajos. (grupo verde)
...
- Gente joven/mediana edad con ingresos elevados. (grupo azul claro)
...
- Gente joven/mediana edad con gastos elevados. (grupo azul claro)
...
- Respecto al grupo azul claro, son gente joven/mediana edad con unos ingresos elevados y gastos elevados.
...
Podemos análizar los items que tenemos en transacciones, como vemos a continuación, tenemos los artistas correspondientes a la primera trasacción, es decir, al primer usuario (si abrimos el fichero comprobamos que tiene el mismo número de artistas y que se corresponde con estos):
```{r}
...
La ejecución anterior nos da una información muy valiosa, hasta ahora las reglas se centraban más en los artistas "más famosos", sin embargo, el lift no está indicando que hay relaciones que no son tan aleatorias como imaginábamos.

Por ejemplo, el grupo the pussycat dolls hacen música más orientado al cabaret y rihanna en ese caso su estilo musical es muy parecido, por lo que es lógico que no haya una aleatoriedad en esa regla.
...
Otro caso similar es el de T.I. con kenye west ambos hacen rap más comercial y son de la misma época, por lo que tiene todo el sentido del mundo que escuchen a ambos artistas, ya que si te gusta ese género y tienes dos artistas similares los vas a escuchar sí o sí.
...
Para realizar este ejercicio hemos decidido usar dos algoritmos de clusterings particionados, el más usado es el kmedias, pero encontramos también el PAM y el CLARA (este último se basa en el PAM pero busca ser eficiente con datasets muy grandes). La principal ventaje de ambos es que no son sensibles a los outliers, mientras que el kmeans sí que lo es.

## PAM

Lo primero que debemos hacer es crear el modelo usando PAM, para poder comparar los resultados, vamos a generar el modelo pero vamos a calcular cuál sería el número de clusters ideal para ese modelo, al igual que hicimos con kmeans:
```{r}
library(factoextra)

fviz_nbclust(df_clientes_copy, pam, method="silhouette")+theme_classic()
```

Como podemos apreciar en la anterior captura el número de clusters ideales para PAM es de 6, al igual que en el kmeans, por lo tanto generamos el modelo:
```{r}
set.seed(0)

clientes_pam_k6 = pam(df_clientes_copy, 6)
clientes_pam_k6
```

## CLARA

El segundo modelo que vamos a crear es el de CLARA, al igual que antes calculamos el número de clusters ideales para el algoritmo de clara:
```{r}
fviz_nbclust(df_clientes_copy, clara, method="silhouette")+theme_classic()
```

En este caso, nos indica que es mejor tener 7 clusters que 6, por lo tanto creamos el modelo con 7 clusters:
```{r}
set.seed(0)

clientes_clara_k7 = clara(df_clientes_copy, 7)
clientes_clara_k7
```

## Comparación de los resultados

Una vez que hemos creado cada modelo según el número de clusters ideales para ese problema, vamos a comparar los resultados que obtenemos a nivel gráfico.

Para ello primero visualizamos las agrupaciones que hace el algoritmo de kmeans respecto a los gastos e ingresos:
```{r}
plot(df_clientes_copy[,c(1,2)], col=clientes_kmeans_k6$cluster, main="Ingresos respecto a la edad")
plot(df_clientes_copy[,c(1,3)], col=clientes_kmeans_k6$cluster, main="Gastos respecto a la edad")
plot(df_clientes_copy[,c(2,3)], col=clientes_kmeans_k6$cluster, main="Gastos respecto a los ingresos")
...
Visualizamos las agrupaciones que hace el algoritmo de PAM respecto a los gastos e ingresos:
```{r}
plot(df_clientes_copy[,c(1,2)], col=clientes_pam_k6$clustering, main="Ingresos respecto a la edad")
plot(df_clientes_copy[,c(1,3)], col=clientes_pam_k6$clustering, main="Gastos respecto a la edad")
plot(df_clientes_copy[,c(2,3)], col=clientes_pam_k6$clustering, main="Gastos respecto a los ingresos")
```

Visualizamos las agrupaciones que hace el algoritmo de CLARA respecto a los gastos e ingresos:
```{r}
plot(df_clientes_copy[,c(1,2)], col=clientes_clara_k7$clustering, main="Ingresos respecto a la edad")
plot(df_clientes_copy[,c(1,3)], col=clientes_clara_k7$clustering, main="Gastos respecto a la edad")
plot(df_clientes_copy[,c(2,3)], col=clientes_clara_k7$clustering, main="Gastos respecto a los ingresos")
```

## Conclusiones

Lo primero que cabe destacar es que el dataset no es lo suficientemente grande como para determinar qué algoritmo es mejor, ya que los cambios que hay entre ellos son muy pequeños.

Aún así la ventaja de usar kmeans respecto a los demás, es que es muy fácil de implementar, pero es vulnerable a los outliers, pero si controlamos eso tendremos un buen algoritmo.

Dado el dataset del ejericio 3 entre el kmeans y pam hay muy pocas diferencias, si nos fijamos detenidamente en cada gráfica solo cambia 2 o 3 puntos entre los clusters que define kmeans y pam, sin embargo, esos puntos que cambia desde mi puntos de vista lo hace mejor el algoritmo pam, ya que tienen más sentido porque esos puntos están más cerca de un clúster que de otro. Pero esto no deja de ser relativo, ya que si el dataset es mayor a lo mejor el algoritmo kmeans se comportaba mejor.

Por otro lado, el algoritmo clara para el mismo conjunto de datos ha necesitado un grupo más que kmeans y pam, por lo que es lógico que haga una mejor división de los datos, es decir, que su agrupamiento esté mejor definido y diferenciado, es por ello, que este algoritmo se comporta similar al pam pero al introducir un grupo más consigue diferenciar mejor los datos, por lo que considero que al fin al cabo el objetivo del clustering es diferenciar grupos de una mejor forma y el modelo clara es el que mejor lo hace.

En resumen, no podemos extraer grandes conclusiones de qué algoritmo es mejor ya que el dataset que tenemos es muy pequeño, si tuviera que elegir entre el kmean y pam, considero que pam ha realizado un mejor agrupamiento, pero entre estos dos y clara, considero que clara es la ganadora ya que introduce un grupo más y hace una mayor distinción de los datos.
