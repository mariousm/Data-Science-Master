...
## Análisis de los efectos principales y posibles interacciones
...
Lo primero que vamos a hacer es crear el dataframe con la información necesaria, respecto a la variable Sexo solo tenemos dos posible valores ("F" o "M") y respecto al RiesgoSM ("1" o "0" ), por lo tanto, vamos a tener 4 combinaciones posibles: F-0, F-1, M-0 y M-1. Creamos el dataframe con la media del CosteFinal en escala logarítmica para cada grupo:
```{r}
df_efectos_principales = data.frame(c("F", "F", "M", "M"), c("0", "1", "0", "1"))
colnames(df_efectos_principales) = c("Sexo", "RiesgoSM")
df_efectos_principales$Sexo = as.factor(df_efectos_principales$Sexo)
df_efectos_principales$RiesgoSM = as.factor(df_efectos_principales$RiesgoSM)

# Calculamos la media para cada grupo
vCosteFinal_F0 = mean(claimNet[claimNet$Sexo == "F" & claimNet$RiesgoSM == "0", "LogCosteFinal"])
vCosteFinal_F1 = mean(claimNet[claimNet$Sexo == "F" & claimNet$RiesgoSM == "1", "LogCosteFinal"])
vCosteFinal_M0 = mean(claimNet[claimNet$Sexo == "M" & claimNet$RiesgoSM == "0", "LogCosteFinal"])
vCosteFinal_M1 = mean(claimNet[claimNet$Sexo == "M" & claimNet$RiesgoSM == "1", "LogCosteFinal"])

# Creamos la nueva variable en el dataframe
df_efectos_principales$Mean_LogCosteFinal = c(vCosteFinal_F0, vCosteFinal_F1, vCosteFinal_M0, vCosteFinal_M1)

# visualizamos el dataframe creado
df_efectos_principales
```

Una vez que tenemos el dataframe que nos interesa, creamos el gráfico de interacción:
```{r}
interaction.plot(df_efectos_principales$Sexo, 
                 df_efectos_principales$RiesgoSM, 
                 df_efectos_principales$Mean_LogCosteFinal, 
                 xlab = "Sexo", 
                 ylab = "Media de CosteFinal", 
                 trace.label = "Riesgo")
```

En el eje X tenemos la variable Sexo, la cual tiene los niveles: "F" - Femenino y "M" - Masculino. En el eje Y tenemos la media del CosteFinal. Además, tenemos 4 casos (F0, F1, M0, M1) están unidos por el factor Riesgo.

Una vez hecha esta aclaración, podemos observar que las líenas son paralelas, no se cortan ni se van a cortar, por lo que no hay interacción entre las variables.

Analizando el gráfico anterior, observamos que si el riesgo es 1, el CosteFinal es mucho mayor que si el riesgo fuera 0. Además, si el sexo es femenino suele suponer un CosteFinal mayor que si el sexo es masculino.

A continuación, vamos a realizar el cálculo del modelo ANOVA y se va a analizar.
...
Lo primero de todo es crear el modelo de las variables LogCosteFinal, el Sexo, el RiesgoSM y la interacción de éstos. Aunque en el anterior apartado hemos visto que no hay interacción, tal y como se indica en el enunciado creamos el modelo con la interacción:
```{r}
modelo_anova = aov(formula = LogCosteFinal ~ Sexo + RiesgoSM + Sexo:RiesgoSM, data = claimNet)

summary(modelo_anova)
```

Analizando el modelo ANOVA creado, vemos que tanto la variable Sexo como RiesgoSM son significantes (tienen Pr(>F) menor que alfa, es decir, Pr(>F) > 0.05), sin embargo, la interacción de estas variables no es significativa (algo que ya sabíamos).

Por otro lado, aquellos factores en los que Pr(>F) < 0.05, rechazamos la hipótesis nula en favor de la alternativa, es decir, que al menos hay una diferencia de la variabilidad de los grupos tanto para Sexo como para RiesgoSM. Esto significa que el CosteFinal sí que se ve afectado por el Sexo y el RiesgoSM.

### Efectos de los niveles del factor

Para medir el efecto de los niveles, hacemos uso de etaSquared:
```{r}
etaSquared(modelo_anova)
```

El eta squared es una medida que nos indica la variabilidad explicada por cada uno de los factores, es decir, por ejemplo la variable Sexo explica el 0.9% de la variabilidad de LogCosteFinal, como vemos es muy poco siendo esta variable la que tiene un mayor eta squared. Por lo tanto, vemos que el LogCosteFinal no depende mucho de estas variables.

### Contraste dos-a-dos

Como la variable Sexo y RiesgoSM son significativas, es decir, hay una diferencia de la variabilidad en sus grupos, tenemos que hacer uso del test de Tukey para saber cómo se distribuyen dichos grupos.

Por lo tanto, lo primero es aplicar dicho test para la variable Sexo:
```{r}
HSD.test(modelo_anova, "Sexo", console = TRUE)
```

En este caso vemos que que tenemos dos grupos, uno relativo al sexo cuando éste es femenino y otro para masculino.

Realizamos el mismo proceso para analizar el RiesgoSM:
```{r}
HSD.test(modelo_anova, "RiesgoSM", console = TRUE)
```

Al igual que antes tenemos dos grupos, uno cuando hay riesgo de padecer enfermedad mental y otro cuando no.

Para termnar con este apartado, vamos a analizar las diferencias dos-a-dos para la interacción de Sexo y RiesgoSM (aunque haya salido que no es significativa). Primero tenemos que quedar con la interacción:
```{r}
interaccion = with(claimNet, interaction(claimNet$Sexo, claimNet$RiesgoSM))
```

Ahora, volvemos a crear el modelo pero solamente para la interacción:
```{r}
modelo_anova_interaccion = aov(formula = LogCosteFinal ~ interaccion, data = claimNet)
```

Finalmente, ejecutamos el HSD test para identificar los grupos:
```{r}
HSD.test(modelo_anova_interaccion, "interaccion", console = TRUE)
```

Vemos que en este caso hay 3 grupos aunque tengamos 4 niveles:

- El grupo A, que incluye F1 y M1, es decir, da igual si eres hombre o mujer, si padeces una enfermedad mental el CosteFinal va a ser más elevado que si no la padeces.

- El grupo B, sexo femenino y sin enfermedad mental.

- El grupo C, sexo masculino y sin enfermedad mental.

En resumen, vemos que lo que más afecta al CosteFinal es si padeces una enfermedad mental, y luego entre si eres hombre o mujer, estas últimas afectan más que los hombres.

### Adecuación del modelo

En este apartado nos indican realizar el análisis visual de normalidad y homocedasticidad.

#### Normalidad de la residuos
...
Respecto a este modelo, vemos que los residuos sí que siguen más o menos una distribución normal, solo para valores muy altos o muy bajos hay un ligerísima desviación, pero podemos apreciar que forma una recta casi perfecta, por lo tanto, seguimos una distribución normal.
...
En este caso tenemos 4 grupos, si el RiesgoSM es 1 o 0 y si el Sexo es F o M, vemos que dentro de cada grupo no están distribuidos los puntos de forma homogénea, por lo que no podemos garantizar la homogeneidad de las varianzas, es decir, estamos en un caso de heterogeneidad de varianzas.

## Interpretación de los resultados

Respecto a este modelo ANOVA, hemos visto que no hay interacción entre los factores, es decir, no existe ninguna relación entre Sexo y RiesgoSM. Sin embargo, analizando los efectos principales, hemos visto que si se padece una enfermedad mental el CosteFinal va a ser mayor que ni no se padece, también si el sexo es femenino tiende a suponer un mayor CosteFinal que si el sexo es masculino.

# Conclusiones

Resumid las conclusiones principales del análisis. Para ello, podéis resumir las conclusiones de cada uno de los apartados.

Respecto a la primera parte de la práctica "Estadística inferencial", nos hemos planetado la pregunta, ¿podemos aceptar que la indemnización a las mujeres supera en más de 1000€ la de hombres? Realizando el contraste de hipótesis correspondiente (contraste de dos muestras independientes sobre la media con varianzas desconocidas y heterogéneas), nos ha dado un pvalor < 2.2e-16 con un nivel de confianza del 95%, por lo que a la vista del contraste, hemos podido rechazar la hipótesis nula en favor de la alternativa, siendo ésta que hay una diferencia en la idemnización de las mujeres respecto a la de los hombres, ya que de media es superada por 1000€.

La siguiente parte de la práctica nos hemos planteado la predicción del CosteFinal dependiendo de variables como Edad, Sexo, Estado, Dependientes, OtrosDepend, Salario, Jornada, HorasSemana, DiasSemana, Clasificacion, RiesgoSM, CosteInicio. Para ello, hemos creado un modelo de regresión lienal cuyo R2 era 0.733 (73.3% de la variabilidad es explicada por el modelo), es decir, no es el mejor modelo pero ajusta bien los datos.

Aunque se han incluido muchas variables independientes en el modelo, realmente significativas son: Edad, Sexo, Estado, Dependientes, Salario, Jornada, DiasSemana, Clasificación, RiesgoSM y LogCosteInicio.

Otra problema que nos hemos planteado era el predecir el Deficit, es decir, si la valoración del coste del siniestro (CosteInicio) es inferior a la indemnización pagada por la compañía (CosteFinal). Para ello, se ha planteado un modelo de regresión logística, siendo 1 si hay déficit o 0 en caso contrario. Como variables independientes de este modelo vamos a tener todas las que teníamos en el modelo anterior. Este modelo nos ha dado una calidad del 8.71%, por lo que no es nada bueno ya que está más cerca del 0% que del 100%.

En este caso las variables independientes significativas para el modelo han sido: Edad, Sexo, Estado, Dependientes, Salario, Jornada, DiasSemana, Clasificación, LogCosteInicio.

Al calcular la matriz de confusión, hemos podido analizar cómo clasificaba los datos. Respecto a la medida sensibilidad de la matriz de confusión, es decir, la proporción de los clasificados correctamente entre todos los que han dado positivo, nos ha dado un valor de 54.45%, es decir, de media clasifica bien, pero este modelo no es muy bueno para predecir valores positivos, es decir, cuando hay un déficit. Respecto a la especificidad (la proporción de casos correctamente clasificados entre todos los que han dado negativo) nos ha dado un valor de 70.15%, es decir, este modelo clasifica mejor cuando no hay déficit que cuando lo hay.

La siguiente parte de la práctia ha estado relacionado con ANOCA, en el primer ejercicio si buscaba saber si había diferencias entre el coste final respecto a la clasificación del siniestro. Para ello, hemos creado el modelo anova y hemos visto que la variable Clasificación era significativa, es decir, que la clasificación del siniestro afecta al coste final de la indemnización.

Hemos realizado el contraste dos-a-dos para saber qué grupos afectaban de mayor a menor medida al coste final de la indemnización, nos ha salido que cuando la clasificación es "Rápido" este grupo genera un mayor coste final, si es "Lento" o "Muy lento" afecta al coste final pero en menor medida (estos dos niveles están en el mismo grupo), si es "Muy rápido" es el último grupo que obtenemos y el que menos afecta al coste final de la indemnización.

Finalmente, nos hemos planteado si había diferencias en el coste final según el sexo y el riesgo de padecer una enfermedad mental. Hemos visto que la interacción de sexo y riesgo no existía pero ambas variables eran significtivas en el modelo anova.

También hemos realizado en contraste dos-a-dos, para saber qué grupos de la interacción afectaban al coste final y en qué medida. En este caso teníamos tres grupos:

- Incluía a la mujeres y hombres con enfermedad mental, es decir, da igual si eres hombre o mujer, si padeces una enfermedad mental el CosteFinal va a ser más elevado que si no la padeces.

- El segundo grupo eran la mujeres sin enfermedad mental. Afecta al coste final menos que el grupo anterior.

- El tercer grupo eran los hombres sin enfermedad mental, este grupo es que el menos afectaba al coste de la indemnización respecto a los demás.

En resumen, hemos visto que lo que más afecta al CosteFinal es si padeces una enfermedad mental, y luego entre si eres hombre o mujer, estas últimas afectan más que los hombres.
