...
<p> Lo primero que debemos hacer es convertir las columnas Club_Joining y Contract_Expiry a tipo date, cabe destacar que de Contract_Expiry conocemos solo el año no el mes y el día en el que acaba el contrato, por lo tanto, he considerado que al indicar el año se refiere a todo el año natural, es decir, hasta el 31 de Diciembre del año X</p>
```{r}
...
```
<p> Como podemos observar, la fecha mínima y la fecha máxima están dentro de los valores que se nos piden entre [1990,2017]. Sin embargo, he tenido que usar na.rm = True, ya que hay un registro cuya fecha está vacía. Dicho registro es el siguiente: </p>
```{r}
...
```
<p> Como bien he mencionado antes, al calcular la fecha del Contract_Expiry consideramos hasta el 31 de Dicimebre, entonces al hacer la consulta para saber si algún registro no cumple que el Contract_Expiry >= Club_Joining, observamos que no hay ningún registro que no la cumpla. </p>
