...
```{r}
relevel_DAY_OF_WEEK = relevel(df_rlgs$DAY_OF_WEEK, ref = "1")
```

Creamos el modelo de regresión logística:
```{r}
modelo_rlgs_DAY_OF_WEEK = glm(formula = delay_SFO ~ factor(relevel_DAY_OF_WEEK), family = binomial(link=logit), data = df_rlgs)
...
```{r}
relevel_AIRLINE = relevel(df_rlgs$AIRLINE, ref = "AA")
```

Creamos el modelo de regresión logística:
```{r}
modelo_rlgs_AIRLINE = glm(formula = delay_SFO ~ factor(relevel_AIRLINE), family = binomial(link=logit), data = df_rlgs)
...
modelo_rlgs_DAY_OF_WEEK_DISTANCE = glm(formula = delay_SFO ~ factor(relevel_DAY_OF_WEEK) + DISTANCE, family = binomial(link=logit), data = df_rlgs_DAY_OF_WEEK_DISTANCE)
...
colnames(df_predict) = c("relevel_DAY_OF_WEEK", "DISTANCE")
