...
Vemos que hay observaciones cuyo atributo ARRIVAL_DELAY no tiene valor, podríamos ignorar estas observaciones pero en nuestro caso no lo vamos a hacer ya que para otros modelos perderíamos información:
```{r}
...
```{r}
cat("El valor del coeficiente de correlación (r) es:", cor(x = df_rls$ARRIVAL_DELAY, y = df_rls$DEPARTURE_DELAY, use = "complete.obs"))
```
...
```{r}
df_rlm = df_rls
df_rlm["DISTANCE"] = df_copy[, c("DISTANCE")]
...
Lo primero que debemos de hacer es crear el dataframe con los datos que nos interesan:
```{r}
df_rlm_cualitativo = df_copy[, c("DEPARTURE_DELAY", "ARRIVAL_DELAY", "LATE_AIRCRAFT_DELAY")]
...
Como podemos observar, la variable LATE_AIRCRAFT_DELAY tiene valores perdidos, pero si eliminamos las observaciones que tienen valores perdidos perderíamos mucha información sobre la variable ARRIVAL_DELAY, es por ello que hemos considerado que lo mejor es mantener dichas observaciones. Los NA no los hemos considerado como 0 o 1 ya que podríamos sesgar al modelo con nuestro criterio, hemos probado tanto este modelo con valores NA y sin NA y nos proporciona el mismo resultado, aún así vamos a mantener los valores NA.
...
En este caso vemos que no todas las aerolíneas son significativas, ya que la aerolínea DL su Pr(>|z|) > 0.05. Respecto al resto de aerolíneas, vemos que todas ellas sí que son significativas, aunque algunas más que otras ya que las aerolíneas AS y VX tienen un nivel de significancia bajo respecto al resto de aerolíneas.
...
Respecto al primer modelo recordamos que todos los días de la semana eran significantes, en el segundo modelo todas las aerolíneas eran significativas menos la aerolínea DL (aún así consideramos que esta variable es significativa, ya que no tiene sentido eliminar los registros cuya aerolínea sea DL, estaríamos perdiendo información), y respecto al tercer modelo los días de la semana sí que son significativos pero la distancia no.

Por lo tanto, vamos a crear un modelo cuyas variables independientes van a ser: DAY_OF_WEEK, AIRLINE y ARRIVAL_DELAY:
```{r}
df_rlgs_DAY_AIRLINE_ARRIVAL = df_rlgs[, c("delay_SFO", "DAY_OF_WEEK", "AIRLINE")]
df_rlgs_DAY_AIRLINE_ARRIVAL$ARRIVAL_DELAY = df_copy[, c("ARRIVAL_DELAY")]

modelo_rlgs_DAY_AIRLINE_ARRIVAL = glm(formula = delay_SFO ~ DAY_OF_WEEK + AIRLINE + ARRIVAL_DELAY, family = binomial(link=logit), data = df_rlgs_DAY_AIRLINE_ARRIVAL)

summary(modelo_rlgs_DAY_AIRLINE_ARRIVAL)
```

Respecto a este modelo vamos que ya no todos los días de la semana son significativoos, ni tampoco todas las aerolíneas, sin embargo, el restraso de llegada sí que es importante, es decir, sí que determina significativamente que se produzca un retraso en la salida.

Para considerar si este modelo es bueno nos fijamos en la devianza residual, y ésta tiene que ser menor que la devianza nula, por lo tanto vemos que esto sí que se cumple. Además, esta devianza residual es la menor obtenida con todos los modelos que hemos creado en este ejercicio, lo cual nos indica que este modelo es mejor y ajusta mejor los datos que cualquiera de los modelos anteriores.

Finalmente, vamos a realizar el cálculo de las OR:
```{r}
exp(coefficients(modelo_rlgs_DAY_AIRLINE_ARRIVAL))
```

Analizado los OR podemos ver el cómo de probable es que ocurra un suceso respecto a que no ocurra, si el coeficiente nos da un valor menor que 1, interpretamos que esa variable reduce la probabilidad del suceso, en nuestro caso el suceso es el retraso en la salida de un vuelo, y por contra si el valor es mayor que 1 se aumenta la probabilidad de que el suceso ocurra.

Por lo tanto, vemos que todos los días de la semana excepto el miércoles, reducen la probabilidad de que haya un restraso en el vuelo. Por otro lado, las aerolíneas AS, F9, HA, US, VX reducen la probabilidad de que haya un restraso, y finalmente, el ARRIVEL_DELAY aumenta la probabilidad de que se produzca un restraso, lo cual es lógico.
...
```{r}
prob=predict(modelo_rlgs_DAY_AIRLINE_ARRIVAL, df_rlgs_DAY_AIRLINE_ARRIVAL, type="response")
roc1=roc(df_rlgs_DAY_AIRLINE_ARRIVAL$delay_SFO, prob, data=df_rlgs_DAY_AIRLINE_ARRIVAL)
plot (roc1)
```
...
Tal y como vimos en la creación del modelo, el modelo del apartado d era el mejor, y observando tanto la gráfica como el área bajo la curva vemos que era verdad, que este modelo ajusta muy bien los datos no como el modelo anterior, es decir, el modelo del apartado d es mejor que el modelo del apartado c.
...
Respecto al segundo ejercicio, hemos realizado diferentes modelos de regresión logística, y hemos ido probando si al introducir determinadas variables el modelo mejoraba o no. Se ha conseguido el mejor modelo en el apartdo d, ya que las tres variables independientes (DAY_OF_WEEK, AIRLINE, ARRIVAL_DELAY) están fuertemente relacionadas con el retraso que se produce en un vuelo.
...
Finalmente hemos realizado el análisis de las curvas ROC tanto para el modelo del apartado c y del apartado d. Respecto a la curva que proporciona el modelo del apartado c, no es la idónea ya que el área que nos da es muy pequeño, es decir, el modelo no ajusta bien los datos. Respecto a la curva que proporciona el modelo del apartado d es muy buena, es decir, el modelo ajusta muy bien los datos.
