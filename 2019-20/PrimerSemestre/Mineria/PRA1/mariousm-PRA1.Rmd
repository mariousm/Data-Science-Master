...
Con el atributo Adhesion pasa algo parecido, aunque no sabemos cómo se mide, a priori según los datos a menor valor de Adhesion menos probable es que tengas cáncer, es decir, más juntas están las células, por lo que vemos que la media y la mediana son bajas, esto nos da a entender que la gran mayoría de datos que tenemos las células están muy juntas, es decir, la mayoría de los casos son benignos.
...
Vemos que tanto el tamaño de la célula como la forma de la misma, están altamente correlacionados positivamente, ya que al aumentar uno aumenta el otro y viceversa, aún así este punto se verá de forma más clara en el siguiente apartado de correlaciones.
...
Al igual que sucedía antes, continua sin seguir una distribución normal, pero en este caso es más probable que estos valores sean pequeños, ya que apenas hay casos en los que sea excesivamente grande.
...
Por otro lado, hay determinadas características como el grosor, tamaño y forma de la célula las cuales a priori paracen que son determinantes para saber si un cáncer es benigno o maligno, también hay otras como el tamaño de la estructura de los nucléolos junto con si el núcleo está rodeado de citoplasma, que nos permite saber si el cáncer es benigno o maligno.
...
Lógicamente al aumentar el tamaño de las células va a aumentar también el tamaño de las células epiteliales, ya que siguen siendo células que se encuentran en la piel. Por lo tanto, si aumenta el tamaño de cualquiera de estas dos variables el riesgo de que padezcamos cáncer de mama es elevado.
...
Por otro lado, una de las correlaciones más significativas respecto a la forma (Cell_Shape) es junto al tamaño de la célula (Cell_Size) tal y como hemos visto en este punto, como esta gráfica ya ha sido representada vamos a mostrar la siguiente correlación más significativa, la cual es Cell_Shape respecto a Epithelial_Cell_Size:
```{r}
...
El análisis de componentes principales lo debemos hacer sobre las variables numéricas, por lo que vamos a hacer uso del dataframe correspondiente:
```{r}
...
Finalmente, para concluir con esta primera práctica vamos a realizar un análisis visual de los datos. Este análisis nos va a permitir darnos respuestas sobre cómo los diferentes atributos afectan al cáncer de mama.

La primer pregunta que nos podemos hacer es, ¿cuántas de las observaciones son cáncer y cuántas no?:
```{r}
library(ggplot2)

ggplot(df_discretizado, aes(`d-Class`, fill = `d-Class`)) + geom_bar() +
  xlab("Tipo de Cáncer") + 
  ylab("Frecuencia") +
  scale_fill_manual(name = "Clase", values=c("#00bfc4","#f8766d")) +
  ggtitle("Frecuencia del cáncer de mama")
```

Tal y como podemos apreciar, hay más casos de cáncer benigno que maligno, para ser más precisos solamente un tercio de las observaciones que tenemos implica que el cáncer de mama sea maligno. De aquí podemos extraer la información de que por norma general el cáncer va a ser benigno, es decir, de todos los casos que se estudian prácticamente tenemos un 66% de que nuestra anomalía en las células no implique un cáncer de mama.

Otro pregunta que nos podemos hacer es, ¿cómo afecta el tamaño y la forma de la célula a que el cáncer sea benigno o maligno?:
```{r}
ggplot(data = df_discretizado, aes(x=`d-Cell_Size`,fill=`d-Class`)) + geom_bar(position = "fill") +
  facet_wrap(~`d-Cell_Shape`) +
  xlab("Tamaño de la célula") +
  ylab("Frecuencia") +
  scale_fill_manual(name = "Clase", values=c("#00bfc4","#f8766d")) +
  ggtitle("Tipo de cáncer respecto al tamaño y forma de la célula") + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.6))
```

El anterior gráfico nos muestra información muy importante, y es que prácticamente si la forma de la célula es de clase2 o clase3, podemos garantizar casi que el cáncer es maligno. Sin embargo, cuando la forma de la célula es pequeña dependiendo de si el tamaño de la misma es pequeño el cáncer lo más probable es que sea benigno, si por contra el tamaño de la célula es mediano o grande es muy probable que el cáncer sea maligno.

Como podemos ver sí que se guarda una relación de que a mayor tamaño de las células más riesgo de que el cáncer sea maligno, y como el tamaño y la forma están altamente correlacionadas positivamente, al aumentar el tamaño de la célula aumenta la forma lo que significa que mayor riesgo de cáncer.

Otra pregunta interesante sería, ¿cómo afecta el grosor de una célula a que las células estén más juntas o no?:
```{r}
ggplot(data = df_discretizado, aes(x=`d-Thickness`,fill=`d-Class`)) + geom_bar(position = "fill") +
  facet_wrap(~`d-Adhesion`) +
  xlab("Grosor de la célula") +
  ylab("Frecuencia") +
  scale_fill_manual(name = "Clase", values=c("#00bfc4","#f8766d")) +
  ggtitle("Tipo de cáncer respecto al grosor y adhesión marginal de la célula") + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.6))
```

Como es lógico y vimos en el apartado de correlaciones, a mayor grosor de la célula mayor riesgo de padecer cáncer. Además, cúanto más juntas estén las células menos riesgo de que el cáncer sea maligno, sin embargo, cuanto más separadas estén las células (la capacidad de adhesión sea más baja) implica un mayor riesgo de padecer cáncer.

Podemos hacernos también la pregunta de cómo el tamaño de las células epiteliales afecta al cáncer:
```{r}
ggplot(df_discretizado, aes(`d-Epithelial_Cell_Size`, fill = `d-Class`)) + geom_bar() +
  xlab("Tamaño del tamaño de la célula epiteliales") + 
  ylab("Frecuencia") +
  scale_fill_manual(name = "Clase", values=c("#00bfc4","#f8766d")) +
  ggtitle("Frecuencia de las células epiteliales respecto al tipo de cáncer")
```

Tal y como podemos ver, a mayor tamaño de la célula epitelial más probable es que el cáncer sea maligno, esto tiene todo el sentido del mundo porque como ya hemos visto cuando aumenta el tamaño de las células el riesgo de padecer un cáncer maligno aumentan.

También es interesante el ver cómo las célulos cuyos núclos no están rodeados de citoplasma afecta al cáncer:
```{r}
ggplot(df_discretizado, aes(`d-Bare_Nuclei`, fill = `d-Class`)) + geom_bar() +
  xlab("Tipo de célula desnuda") + 
  ylab("Frecuencia") +
  scale_fill_manual(name = "Clase", values=c("#00bfc4","#f8766d")) +
  ggtitle("Frecuencia de las células desnudas respecto al tipo de cáncer")
```

Como podemos observar, si el citoplasma que rodea al núcleo no está presente prácticamente podemos asegurar de que el cáncer es maligno, sin embargo, cuando el citoplasma está presente (esto no es una anomalía) podemos ver que en la gran mayoría de casos el cáncer es benigno. Finalmente, cuando sí que hay citoplasma pero éste es poco, es un poco más probable que el cáncer sea maligno.

Otra pregunta interesante es el ver cómo el cáncer es afectado según la textura del núcleo de la célula:
```{r}
ggplot(df_discretizado, aes(`d-Bland_Chromatin`, fill = `d-Class`)) + geom_bar() +
  xlab("Tipo de textura del núcleo") + 
  ylab("Frecuencia") +
  scale_fill_manual(name = "Clase", values=c("#00bfc4","#f8766d")) +
  ggtitle("Frecuencia de la textura del núcleo respecto al tipo de cáncer")
```

Como era de esperar, cuánto más burda sea la textura del núcleo de la célula, más probable es que el cáncer sea maligno, es llamativo ver que cuando es uniforme la gran mayoría de las veces el cáncer es benigno.

Finalmente, nos podemos hacer la siguienre pregunta, ¿la estructura de los nuclélos afecta al cáncer?
```{r}
ggplot(df_discretizado, aes(`d-Normal_Nucleoli`, fill = `d-Class`)) + geom_bar() +
  xlab("Tamaño de estructura de los nucléolos") + 
  ylab("Frecuencia") +
  scale_fill_manual(name = "Clase", values=c("#00bfc4","#f8766d")) +
  ggtitle("Frecuencia del tamaño de los nucléolos respecto al tipo de cáncer")
```

Como podemos apreciar cuanto más grande sea la estructura del nucléolo más probabilidades tenemos de que el cáncer de mama sea maligno. Al igual que antes cabe destacar que, el número de casos cuyo cáncer es benigno cuando el nucléolo es pequeño es muy elevado.

### Conclusiones

Resumiendo, hemos visto que dos tercios de los datos son sobre observaciones cuyo tumor es benigno, sin embargo, el otro tercio restante implica que el tumor es maligno.

Por otro lado, al comparar el tamaño de la célula y su forma respecto al tipo de cáncer, hemos podido ver que a mayor tamaño y forma más riesgo de padecer cáncer.

Analiando el grosor junto con la capacidad de adherirse las células, vemos que a mayor grosor más riesgo de padecer cáncer, y cuanto menor sea la capacidad de adherirse las células (más separadas están entre sí), significa que probablemente el cáncer sea maligno.

Siguiendo con el análisis, estudiando el tamaño de las células epiteliales, lógicamente a mayor tamaño mayor riesgo. Esto mismo sucedía con los núcleos denudos, es decir, si no hay citoplasma rodeando al núcleo mayor riesgo.

Finalmente, respecto a la textura del núcleo, cuanto más burda sea la textura mayor riesgo de padecer cáncer tenemos, y si el tamaño de los nucléolos es grande el riesgo sigue aumentando.
