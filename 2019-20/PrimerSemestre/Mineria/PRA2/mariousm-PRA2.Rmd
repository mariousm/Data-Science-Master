...
Una vez analizado el dataset, tenemos que crear el modelo correspondiente. Cabe destacar que el algoritmo que se va usar para este ejercicio es PAM, tanto kmeans como PAM son parecidos, pero PAM su medida es la mediana da las observaciones del clúster (medoid), mientra que en kmeans se usa la media.
...
La silueta nos indica cómo de similar es un elemento respecto a los de su propio grupo y cómo de diferente respecto a los demás, cuanto más próximo esté el valor a 1 mejor es el modelo, esto significa que una observación está muy relacionada con los de se grupo y a su vez es muy deferente respecto a los demás.

Por lo tanto, obtenemos una silueta apróximada a 0.60, es decir, el modelo agrupa bien la información pero la podría mejorar.
...
Además, la silueta media del primer clúster es de 0.28 y del segundo 0.76, es decirk, confirmamos de forma numérica que el segundo clúster es mejor.
...
En cuanto a las reglas de asociación, el mayor riesgo que presentamos es que haya reglas insiginificantes, ya sea porque son redudantes, es decir, una misma regla se puede obtener de otras, también porque sea una regla muy aleatoria, y cuando estamos analizando un problema no queremos aleatoriedad, sino que la información que obtenemos sea certera.
