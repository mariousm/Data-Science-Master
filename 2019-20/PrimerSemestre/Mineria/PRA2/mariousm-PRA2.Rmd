...
El significado de este dataset para una variable en específico sería el siguiente, si estudiamos el tamaño de la célula (Cell_Size), a mayor valor más riesgo de que sea un tumor maligno.

Si se necesita más información sobre el significado de las variables, análisis de las mismas, análisis del dataset en general, se puede hacer la lectura de la práctica 1.
...
Algo curioso del anterior gráfico es que podemos tener un grosor muy elevado, pero que la célula sea muy pequeña y esto no significa para nada que tengamos cáncer, es decir, es muy influyente el cómo es el tamaño de la célula para dictaminar si ésta es cancerígena o no.
...
Por último respecto al grosor, vamos a comprobar si Bare_Nuclei afecta, es decir, si las células que están rodeadas de citoplasma y las que no dependen del grosor o no:
```{r}
...
En este ejercicio tenemos como objetivo seguir obteniendo respuestas para nuestro dataset, a partir de un modelo no supervisado y que nos permita determinar qué es lo que hace que un tumor sea benigno o maligno. En otras palabras, seguimos hablando de clustering, pero en este caso vamos a crear otro modelo haciendo uso de un algoritmo diferente, para ver qué modelo se comporta mejor, y si podemos obtener información que antes no.

### Conjunto de datos

En este caso, para poder hacer una comparativa entre el modelo del ejercicio anterior y éste, debemos de usar los mismos datos, aunque podríamos usar el dataframe creado para el ejercicio uno, vamos a volverlo a crear para este problema:
```{r}
df_ejer2 = df_copy[, 2:10]

str(df_ejer2)
```

Vemos que tenemos todos los datos de forma correcta, es decir, no se encuentran ni la variable ID (no nos aporta información), ni la variable Class (al hacer clustering no podemos tener en cuenta la variable objetivo, ya que buscamos encontrar patrones de agrupación).
...
Seguimos manteniendo todas las variables con valores acotados, es decir, como valor mínimo va a ser 1 y como máximo 10. A medida que aumenta el valor de cada variable, tenemos más riesgo de que el tumor sea maligno, en caso contrario estaríamos en un caso benigno.

### Creación del modelo

Una vez analizado el dataset, tenemos que crear el modelo correspondiente. Cabe destacar que el algoritmo que se va usar para este ejercicio es PAM, tanto kmeans como PAM son parecidos, pero PAM su medida es la mediana da las observaciones del clúster (medoid), mientras que en kmeans se usa la media.

Para usar PAM, hay que definir de entrada cuántos clústers vamos a tener, es decir, lo mismo que sucedía con kmeans, en nuestro caso sabemos que solo hay dos grupos, o tienes cáncer o no tienes, pero para comprobarlo vamos a ejecutar la siguiente línea, la cual nos devuelve el número de grupos ideal para PAM:
```{r}
fviz_nbclust(df_ejer2, pam, method="silhouette")+theme_classic()
```

En este caso, seguimos viendo que el número de clústers ideales para nuestro modelo son dos: o el tumor es benigno o es maligno.

Una vez que tenemos claro los grupos, definimos el modelo:
```{r}
set.seed(0)

modelo_ejer2 = pam(df_ejer2, 2)
```

### Análisis y comparación de los modelos

Antes de realizar la comparación de los modelos, vamos a ver cómo de bueno el modelo que generamos con PAM. Para ello, vamos a realizar el análisis de los clústers que genera y cuál es el valor de la silueta.

Por lo tanto, para ver cómo de bueno es el modelo vamos a calcular la silueta:
```{r}
silueta_ejer2 = silhouette(modelo_ejer2)

cat("El valor de la silueta para este ejercicio es: ", mean(silueta_ejer2[,3]))
```

La silueta nos indica cómo de similar es un elemento respecto a los de su propio grupo y cómo de diferente respecto a los demás, cuanto más próximo esté el valor a 1 mejor es el modelo, esto significa que una observación está muy relacionada con los de se grupo y a su vez es muy diferente respecto a los demás.

Por lo tanto, obtenemos una silueta aproximada a 0.60, es decir, el modelo agrupa bien la información pero la podría mejorar.

Una vez que sabemos la silueta, vamos a vér qué grupos ha generado a nivel visual:
```{r}
eclust(df_ejer2, "pam", k = 2, hc_metric="euclidean")
```

Tal y como podemos ver agrupa los datos medianamente bien, pero es verdad que hay una zona en la que se solapan ambos clústers, es decir, tenemos una zona de conflicto en el que esos datos podrían pertenecer a un grupo o a otro. Es por ello, que el valor de la silueta no es muy elevado, es decir, es un bueno modelo pero podría ser mejor.

Para ver cómo de bueno es el modelo, vamos a compararlo con el kmeans del ejercicio anterior, lo primero de todo es ver cómo agrupa la información kmeans:
```{r}
eclust(df_ejer1, "kmeans", k = 2, hc_metric="euclidean")
```

Como podemos observar, la agrupación que hace kmeans es muy similar a la que hace PAM, no se sabe a ciencia cierta qué modelo agrupa mejor la información, es decir, quién es mejor. Para ello, vamos a realizar de nuevo el cálculo de la silueta pero en este caso para ambos modelos.

Respecto al modelo kmeans del primer ejercicio, obtenemos la siguiente silueta:
```{r}
fviz_silhouette(silhouette(modelo_ejer1$cluster, dist(df_ejer1)))
```

El modelo anterior nos genera una silueta media de 0.6, es decir, el mismo valor que el modelo generado en este ejercicio. Respecto al modelo kmeans, vemos que el resultado de clustering es mejor con el segudo grupo.

Además, la silueta media del primer clúster es de 0.28 y del segundo 0.76, es decir, confirmamos de forma numérica que el segundo clúster es mejor.

Realizamos lo mismo para el modelo PAM:
```{r}
fviz_silhouette(silhouette(modelo_ejer2$clustering, dist(df_ejer2)))
```

En este caso, el grupo 1 tiene un tamaño de 467 y su silueta media es de 0.75, un pelín menor que el grupo de mayor tamaño del kmeans, cuyo tamaño era de 465 y tenía una media de silueta en dicho grupo de 0.76, por lo que el modelo se comportaba mejor con el algoritmo kmean que con PAM respecto a este grupo. Por otro lado, el segundo grupo (grupo más pequeño) sigue teniendo la misma silueta, con un valor de 0.28, es decir, en este caso ambos modelos se comportan igual.

En resumen, vemos que ambos modelos se comporta igual, pero es verdad que el kmeans en este caso funciona un poco mejor que el PAM. Además, la agrupación que hace de datos es muy similar, solo se diferencias en dos observaciones, es decir, con kmeans el grupo más pequeño tiene dos observaciones más, y con PAM el grupo más grande es el que tiene dichas observaciones.

### Conclusiones

En este ejercicio hemos creado otro modelo de clustering, pero en este caso basándonos en PAM en vez de kmeans. La diferencia entre ambos modelos es que PAM se basa en la mediana y kmeans en la media, a pesar de eso hemos visto que ambos modelos eran similares, es decir, no eran perfectos pero sí que agrupaban bien los datos.

Respecto al modelo de kmeans realizado en el ejercicio 1, nos daba un silueta próxima a 0.6, mientras que en este ejercicio, PAM nos daba también una siluete media de 0.6. Es decir, ambos modelos son buenos y apenas hay diferencia entre ellos.

Respecto a la información que obtenemos al analizar los clústers de PAM frente a los de kmeans, sigue siendo la misma, es decir para valores bajos en cada atributo estamos en un caso de tumor benigno, mientras que valores altos tiende a implicar un tumor maligno.

Aunque en este ejercicio no se ha realizado las conclusiones a nivel visual como hicimos en el ejercicio anterior, analizando y evaluando el modelo PAM, hemos visto que es prácticament el mismo modelo que kmeans con la diferencia en la agrupación de dos observaciones, es decir, con kmeans el grupo más pequeño tiene dos observaciones más, y con PAM el grupo más grande es el que tiene dichas observaciones, por lo tanto, la información que obtenemos es la misma que en el ejercicio anterior.
...
En nuestro caso sí que tiene sentido aplicar un método de generación de reglas asociadas. Recapitulando, tenemos un dataset sobre el cáncer de mama, en él hay variables como el tamaño de las células, su forma, si hay citoplasma o no dentro de la célula... Todas estas variables nos permiten saber si el tumor que estamos analizando es benigno o maligno, en otras palabras, si es cáncer o no.
...
Lo primero de todo es que al usar el algoritmo apriori, todas las variables tienen que ser categóricas, en otras palabras factores. Para ello, en la primera parte de la práctica se creó un dataset discretizado, dicho dataset contenía tanto las variables del dataset original más las discretizadas, es por ello que solo nos vamos a quedar con las discretizadas:
```{r}
...
Una vez que ya tenemos la información deseada, convertimos el datset al conjunto de tipo transacciones para poder aplicar el método apriori:
```{r}
...
Una vez aclarado esto vamos a hacer uso del método apriori para obtener las reglas, como no se indica qué parámetros debemos de hacer uso vamos a usar para el soporte un 0.1 y para la confianza un 0.8. Se han elegido estos parámetros ya que son los que usa por defecto el método apriori:
```{r}
...
- Otro aspecto es la cantidad de citoplasma que hay en la célula, y vemos que si es Ausente, es decir, no hay o apenas hay citoplasma, suele indicar que tenemos cáncer.

- El grosor de la célula también está implícito en si el tumor es maligno o no, ya que si el grosor es Grueso es más probable que tengamos un cáncer.
...
En este ejercicio se nos planeta dos preguntas, la primera de ellas, ¿qué limitaciones tiene el dataset?, y la segunda, ¿qué riesgos presenta?

### Limitaciones del dataset

Una de las principales limitaciones que presenta este dataset, es que viene información sobre las células, pero ésta no es información real, es decir, no sabemos el tamaño de la célula en nanómetros, sino que tenemos un valor del 1-10 que nos indica cómo de grande o pequeña es la célula, cuanto más pequeño sea el valor más pequeño es el tamaño.

Esta limitación puede llegar ser como tal si tenemos otros objetivos, en nuestro caso el objetivo principal era identificar a grandes rasgos el cómo se clasificaba el cáncer y qué lo caracterizaba, por lo que saber si una célula es 1 nanómeto o 10 nanómetros nos da más igual, ya que a grandes rasgos tenemos caracterizada la misma. Pero si nuestro objetivo fuera diferente, como por ejemplo, predecir el grosor del tumor a partir de otros atributos, vemos que en este caso el dataset seleccionado no nos sirve, es decir, no podemos saber cuál es el grosor teniendo solo como información si una célula es grande o pequeña en una escala del 1 al 10, podríamos predecir el valor dentro de esa escala pero no tendríamos un valor real del problema.

Otra limitación respecto al contenido en sí, es que tenemos la información muy sintetizada, es decir, en el dataset nos viene un atributo para el tamaño de la célula, pero ésta a su vez la podría definir como dos atributos o como tres, anchura, altura y profundidad. En nuestro caso vemos que no es así, por un lado está bien porque simplifica el problema, pero por otro estamos perdiendo mucha información, es decir, a lo mejor el cáncer viene dictaminado por la profundiad de la célula y no por el tamaño en sí.

Otro aspecto a tener en cuenta es la información sesgada, en nuestro caso podemos tenemor muchos sesgos:

- Los profesionales que normalizaron el dataset en una escala del 1 al 10, cómo fue la decisión de saber cuándo un tumor tiene un atributo con valor 2 y no con valor 3, es decir, la información que tenemos está influenciada por cómo se han definido los atributos, y éstos no dependen de nosotros, por lo que las conclusiones que podemos obtener con los modelos no son las correctas.

- Otro sesgo importante es la cantidad de tumores benignos y malignos que tenemos en el propio dataset, es decir, cuando hacemos un modelo de clasificación, la "línea" que nos permite separar las observaciones de una clase y de otra depende de la cantidad de registros que haya, si tenemos 700 casos que son benignos pero 1 que es maligno, al modelo no le interesa aprender cuando una observación es un tumor maligno, predice que siempre el tumor es benigno y hay menos error. En nuestro dataset tal y como se analizó en la práctica 1, hay el doble de casos cuyo tumor es benigno respecto a maligno, vemos entonces que el dataset no está del todo balanceado, y esto nos puede generar un riesgo a la hora de obtener las conclusiones.

- También podemos tener la información sesgada porque no sabemos el origen de los datos, las observaciones del dataset pueden ser a nivel mundial, a nivel nacional, de mujeres, de niños, de hombres, de europeos, africanos... Es decir, al no saber de dónde procede la información no podemos generalizar con este dataset, por lo tanto, las conclusiones que obtenemos son solo para los datos que tenemos.

- ...

### Riesgos del uso del modelo

Los principales riesgos que nos encontramos al usar los modelos, son aquellos que tal y como hemos visto en el apartado anterior nos sesgan la información.

Si los datos de entrada que proporcionamos a cada modelo no son idóneos, ya sea porque están mal balancedas las clases, no sabemos el origen de los datos... Al hacer uso del modelo estaremos cometiendo un gran error.

Respecto a los modelos de clustering, el principal riesgo es no saber identificar los grupos, pero en nuestro caso esto está bien definido ya que siempre son dos, o el tumor es benigno o es maligno. También pueder ser que haya un solapamiento de grupos, haciendo que cada vez muchas observaciones sean tumores benignos y en otras ocasiones tumores malignos, es decir, depende mucho de la aleatoriedad del algoritmo. Tal y como vimos en la práctica, había una pequeña diferencia entre usar kmeans que es vulnerable a los outliers, ya que se basa en la media, y PAM que se centra en la mediana por lo tanto no es vulnerable a los valores extremos.

En cuanto a las reglas de asociación, el mayor riesgo que presentamos es que haya reglas insignificantes, ya sea porque son redudantes, es decir, una misma regla se puede obtener de otras, también porque sea una regla muy aleatoria, y cuando estamos analizando un problema no queremos aleatoriedad, sino que la información que obtenemos sea certera.

Otro aspecto interesante de las reglas de asociación, es que se basan en la información que le hemos proporcionado, por lo que si el día de mañana introducimos otro dataset, las reglas que obtenemos pueden cambiar, es decir, este modelo nos sirve para un conjunto de datos en particular, solamente para analizar la información de un caso muy específico.

Finalmente, en cuanto a los modelos de clasificación, que un modelo sea bueno o no depende mucho de los datos de entrada y del entreamiento del mismo, es decir, si los datos que le proporcionamos no están balanceados, a la hora de entrenar el modelo vamos a tener un sobreajuste para una determinada clase, y esa clase va a ser de la que haya más datos en el dataset, por lo tanto, para nuevas observaciones no va a ser capaz el modelo de clasificar correctamente. Otro problema, es que para poder aplicar un modelo de clasificación necesitamos la variable objetivo, y pude ser que haya observaciones que no contengan información de la misma, por lo que si no las incluímos en el modelo estamos perdiendo información, y si las incluimos tenemos que predecir previamente dicho valor, y esto puede aumentar el error.
